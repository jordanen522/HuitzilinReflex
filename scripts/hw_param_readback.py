...
an untested copy.
  ...
  ./scripts/hw_param_readback.py --expected other.parm

The default connection is mavlink-router's local tools endpoint
(hardware/mavlink-router.conf), so this runs alongside the flight stack. With
the router stopped, pass the serial device instead:
  ./scripts/hw_param_readback.py --connection /dev/serial/by-id/usb-ArduPilot-...
...
DEFAULT_BAUD = 115200          # serial only; ignored for udp connections
DEFAULT_CONNECTION = "udpin:0.0.0.0:14554"   # router [UdpEndpoint tools]
...
def fetch(connection, timeout_s, baud=DEFAULT_BAUD):
    ...
    # baud is explicit: mavutil defaults to 57600 on a serial device, and a
    # mismatch reports a dead FC on a perfectly good link.
    ...
    # break, and the normal return each left the port claimed, and the *next*
    # tool then reports a dead flight controller.
    try:
